                        ...
                        <div className={`listing-card-four overflow-hidden d-flex align-items-end position-relative z-1 ${item.item_bg_img}`}>
                           ...
                           <div className="property-info tran3s w-100">
                              <div className="d-flex align-items-center justify-content-between">
                                 <div className="pe-3">
                                    <Link href={`/listing_details_01?id=${item.id}`} className="title fw-500 tran4s">{item.title}</Link>
                                    <div className="address tran4s">{item.address}</div>
                                 </div>
                                 <Link href={`/listing_details_01?id=${item.id}`} className="btn-four inverse"><i className="bi bi-arrow-up-right"></i></Link>
                              </div>
