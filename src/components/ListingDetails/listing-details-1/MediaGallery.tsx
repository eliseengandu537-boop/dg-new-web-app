...
interface Props {
  style?: any;
  /** Dynamic gallery images from the API */
  images?: string[];
  /** Featured / hero image from the API */
...
const MediaGallery = ({ style, images, featuredImage }: Props) => {
  // Build the ordered list: featuredImage first, then the rest of the gallery
  const apiImages: string[] = [
    ...(featuredImage ? [featuredImage] : []),
    ...(images || []),
  ];
  const hasApi = apiImages.length > 0;
  const thumbs = apiImages.slice(0, 4);

  // Static fallbacks
  const staticBig = [bigCarousel_1, bigCarousel_2, bigCarousel_3, bigCarousel_4];
  const staticSmall = [smallCarousel_1, smallCarousel_2, smallCarousel_3, smallCarousel_4];

  return (
    <div className="media-gallery mt-100 xl-mt-80 lg-mt-60">
      <div id="media_slider" className="carousel slide row g-3 align-items-stretch">
        {/* Main image */}
          ...
          <div className="bg-white shadow4 border-20 p-20 h-100">
            <div className="position-relative z-1 overflow-hidden border-20" style={{ height: mainGalleryHeight, minHeight: 180 }}>
              <div className="img-fancy-btn border-10 fw-500 fs-16 color-dark">
                See all {hasApi ? apiImages.length : 37} Photos
                <Fancybox options={{ Carousel: { infinite: true } }}>
                  {hasApi
                    ? apiImages.map((src, i) => (
                        <a key={i} className="d-block" data-fancybox="img2" href={src}></a>
                      ))
                    : (["1", "2", "3"] as string[]).map((t, i) => (
                        <a key={i} className="d-block" data-fancybox="img2" href={`/assets/images/listing/img_large_0${t}.jpg`}></a>
                      ))}
                </Fancybox>
              </div>
                ...
                {hasApi
                  ? apiImages.map((src, i) => (
                      <div key={i} className={`carousel-item h-100 ${i === 0 ? "active" : ""}`}>
                        ...
                        <img src={src} alt="" className="w-100 border-20" style={{ height: "100%", minHeight: 180, objectFit: "cover" }} />
                      </div>
                    ))
                  : staticBig.map((carousel, i) => (
                      <div key={i} className={`carousel-item h-100 ${i === 0 ? "active" : ""}`}>
                        <Image src={carousel} alt="" className="w-100 border-20" style={{ height: "100%", minHeight: 180, objectFit: "cover" }} />
                      </div>
                    ))}
              </div>
        ...
        <div className="col-lg-3 d-none d-lg-flex flex-column gap-3">
              {(hasApi ? thumbs : staticSmall).map((src, i) => (
            <button
              key={i}
              type="button"
            ...
            >
              <div className="bg-white shadow4 border-15 overflow-hidden" style={{ height: "100%" }}>
                {hasApi
                  ? /* eslint-disable-next-line @next/next/no-img-element */
                    <img src={src as string} alt="" className="w-100 h-100" style={{ objectFit: "cover", display: "block" }} />
                  : <Image src={src as any} alt="" className="w-100 h-100" style={{ objectFit: "cover", display: "block" }} />
                }
              </div>
            </button>
          ))}
